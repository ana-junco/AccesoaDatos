    ...
    public partial class frmEliminar : Form
    {
        ...
        private DataTable getpersona(string cedula = "")
        {
            ...
            if (cedula == "")
            {
                ...
                sql += "from personas order by apellido, nombres";
            }
            else
            {
                sql = "select cedula, apellidos, nombres, upper(apellido+ ' ' + nombre) as nombreCompleto, fechadenacimiento, peso ";
        ...
        {
            if (MessageBox.Show("¿DESEAS ELIMINAR AL USUARIO?", "CONFIRMAR", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                conexion.Open();
                ...
                conexion.Close();
                MessageBox.Show(" REGISTRO ELIMINADO", "ELIMINO", MessageBoxButtons.OK, MessageBoxIcon.Error);
                DataTable dt = getpersona();
                this.comboPersona.DataSource = dt;
                this.comboPersona.DisplayMember = "nombreCompleto";
                this.comboPersona.ValueMember = "cedula";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("¿DESEAS ACTUALIZAR?", "CONFIRMAR", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    conexion.Open();
                    string actualizar = "UPDATE personas SET apellidos=@apellidos, nombres=@nombres, fechadenacimiento=@fechadenacimiento, peso=@peso WHERE cedula=@cedula";
                    SqlCommand cmd2 = new SqlCommand(actualizar, conexion);
                    cmd2.Parameters.AddWithValue("@cedula", this.txtCedula.Text);
                    cmd2.Parameters.AddWithValue("@apellidos", this.txtApellidos.Text);
                ...
                {
                    MessageBox.Show(e1.Message.ToString(), "CERRAR EL PROGRAMA Y REEMPLAZE LA COMA POR EL PUNTO EN EL PESO");
